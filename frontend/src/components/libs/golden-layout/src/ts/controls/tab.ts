import { CHANGE_TITLE_EVENT } from "../../../../../../lib/model/context";
import {
  CHANGE_TITLE,
  REMOVE_PANEL_EVENT,
} from "../../../../../../lib/model/site";
import { Component } from "../../../../../layout/golden-layout/app";
...
export class Tab {
  ...
  /** @internal */
  private readonly _tabClickListener = (ev: MouseEvent) =>
    this.onTabClickDown(ev);
  /** @internal */
  private readonly _tabTouchStartListener = (ev: TouchEvent) =>
    this.onTabTouchStart(ev);
  /** @internal */
  ...
  /** @internal */
  private readonly _dragStartListener = (x: number, y: number) =>
    this.onDragStart(x, y);
  /** @internal */
  private readonly _contentItemDestroyListener = () =>
    this.onContentItemDestroy();
  /** @internal */
  ...
  constructor(
    ...
    this._closeElement.innerText = "X";
    ...
    document.addEventListener(
      CHANGE_TITLE_EVENT,
      this._changeTitleListener.bind(this)
    );

    const reorderEnabled =
      _componentItem.reorderEnabled ??
  ...
  _changeTitleListener(e: CustomEvent<Component>) {
    if (e.detail.internalIds.includes(this.componentItem.componentId)) {
      this.setTitle(e.detail.name);
    }
  }
  ...
  destroy(): void {
    ...
    this._closeElement?.removeEventListener(
      "touchstart",
      this._closeTouchStartListener
    );
