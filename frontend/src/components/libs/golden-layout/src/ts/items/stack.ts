import { REMOVE_PANEL_EVENT } from "../../../../../../lib/model/site";
...
export class Stack extends ComponentParentableItem {
  ...
  constructor(
    ...
    if (configContent.length !== 1) {
      ...
      componentHeaderConfig = (
        firstChildItemConfig as ResolvedHeaderedItemConfig
      ).header; // will be undefined if not component (and wont be stack)
    }
    ...
    const show =
      this._headerConfig?.show ??
      componentHeaderConfig?.show ??
    ...
    const close =
      this._headerConfig?.close ??
      componentHeaderConfig?.close ??
  ...
  override removeChild(contentItem: ContentItem, keepChild: boolean): void {
    ...
    document.dispatchEvent(
      new CustomEvent<string>(REMOVE_PANEL_EVENT, {
        detail: componentItem.componentId,
        composed: true,
        bubbles: true,
      })
    );
  ...
  override onDrop(contentItem: ContentItem, area: ContentItem.Area): void {
     ...
     */
    const isVertical =
      this._dropSegment === Stack.Segment.Top ||
      this._dropSegment === Stack.Segment.Bottom;
    const isHorizontal =
      this._dropSegment === Stack.Segment.Left ||
      this._dropSegment === Stack.Segment.Right;
    const insertBefore =
      this._dropSegment === Stack.Segment.Top ||
      this._dropSegment === Stack.Segment.Left;
    ...
    if (hasCorrectParent) {
      ...
      const itemConfig = ResolvedItemConfig.createDefault(
        type
      ) as ResolvedItemConfig;
  ...
  private highlightHeaderDropZone(x: number): void {
    ...
    while (tabCount < visibleTabsLength) {
      const visibleTabElement = tabsContainerElementChildNodes[
        tabIndex++
      ] as HTMLElement;
    ...
    if (visibleTabsLength === 0) {
      ...
      const tabDropPlaceholderRectTop =
        tabDropPlaceholderRect.top + document.body.scrollTop;
