import { CSSResultGroup, LitElement, PropertyValueMap, TemplateResult  } from 'lit-element';
...
import { BodyView, BookeraPlusView, ConfigUpdateEvent, FrontMatterView, OtherViews, Outline, SlTreeChangeEvent, StudioPageData, StudioPageView, TypeSettingView, defaultStudioPageData, intermediateStudioPageContext, studioPageContext, typeStudioPageViewKeys } from '../../lib/model/context';
import { titleCase } from '../../lib/model/meta';
import { ContextConsumer } from '@lit/context';
import { styleMap } from 'lit/directives/style-map.js';
import { CREATE_GRAPH_VIEW_EVENT, NEW_PANEL_EVENT, NewPanelEventType } from '../../lib/model/site';
...
export const defaultParentOutline: Outline  = {
	...
	export const expandOutlineRecursively = (parentOutlineSubtype: OutlineSubtype, headersLeft: Outline[]): OutlineSubtype  => {
...
export class CreateSidePanelElement extends LitElement {
	...
	matchCurrentViewToExpand() {
		...
		const view = this._studioPageData.value.currrentView.replaceAll(" ", "").toLowerCase();

		if (Object.keys(TypeSettingView).map((key) => {
			return key.toLowerCase()
		}).includes(view)) {
			return "typesetting"
		}
		if (Object.keys(FrontMatterView).map((key) => {
			return key.toLowerCase()
		}).includes(view)) {
			return "frontmatter"
		}
		if (Object.keys(BookeraPlusView).map((key) => {
			return key.toLowerCase()
		}).includes(view)) {
			return "bookeraplus"
		}
	...
	getDetails(key: typeStudioPageViewKeys) {
		...
		${Object.keys(inside).map((key) => {
			return html`
			<sl-tree-item  ?selected=${key.toLowerCase() === this._studioPageData.value?.currrentView?.toLowerCase()}>
				${titleCase(key)}
			</sl-tree-item>
			`;
		})}
	...
	dragElement(e) {
		console.log(e, "this functionality is not done. Let's make the graph, lol!")
	...
	constructOutlineHTML(outlineSubtype: OutlineSubtype): TemplateResult {
		...
		return html`<sl-tree-item expanded><a href=${"/studio#" + self.id}>${self.name}</a>${outlineSubtype.children.map((child => this.constructOutlineHTML(child)))}</sl-tree-item>`
	}
	...
	expandOutline() {
		...
		const outlineSubtype = convertOutlineToTree(this._studioPageData.value.content.config.outline)
	...
	changeSelection(e: any) {
		...
		if (a.href.includes("studio")) {
			...
			const titleElement = this._studioPageData.value?.siteConfig.editorDiv?.querySelector("iframe")?.contentDocument?.getElementById(titleID)!
		...
		this.dispatchEvent(new CustomEvent(SlTreeChangeEvent, {detail: slTreeItem.innerText, bubbles: true, composed: true}))
	}
	...
	renderBodyButtons() {
		...
		<div class="button-box">
			<sl-tooltip content="Open graph view"> 
				<sl-icon-button name="diagram-3" label="diagram" @click=${() => { 
					this.dispatchEvent(new CustomEvent<NewPanelEventType>(NEW_PANEL_EVENT, { detail: { type: OtherViews.BodyGraphView, name: "Graph" }, composed: true, bubbles: true}));
					}}></sl-icon-button>
			</sl-tooltip> 
		</div>
			`
		...
		return html`
			<section class="h-[100vh]">
				<sl-tree  @sl-selection-change=${this.changeSelection}> 
					...
					return html`
						<sl-tree-item class="tree-item" @dblclick=${() => {

							this.dispatchEvent(new CustomEvent<NewPanelEventType>(NEW_PANEL_EVENT, { detail: { type: StudioPageView.Body, name: "Body" }, composed: true, bubbles: true}));

						}}  ?selected=${key.toLowerCase() === this._studioPageData.value?.currrentView?.toLowerCase()}>
							${titleCase(key)}
