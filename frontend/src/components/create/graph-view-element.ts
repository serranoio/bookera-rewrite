...
import { IntermediatetudioPageChanges, Outline, StudioPageChanges, StudioPageData, defaultStudioPageData, extractContentsUnderOutline, findNextMode, getBodyElement, intermediattudioPageContext, studioPageContext, updateOutlineInBody } from '../../lib/model/context';
import FroalaEditor from 'froala-editor';
import "../libs/graph/graph-element"
import "../libs/graph/node-element"
import { styleMap } from 'lit/directives/style-map.js';
import { Direction, PositiningStrategy, attachPositioning } from '../libs/graph/graph-utils';
import { OutlineSubtype, convertOutlineToTree, defaultParentOutline, expandOutlineRecursively } from './create-side-panel-element';
import { defaultGraphData } from '../libs/graph/graph-element';
import Dagre from '@dagrejs/dagre';
import { UPDATE_GRAPH_EVENT } from '../../lib/model/site';
import { StudioSettings, defaultSettings, studioSettingsContext, ExtractContentsMode, SettingsChanges } from '../../lib/model/settings';
...
export class GraphViewElement extends LitElement {
	...
	_studioPageData = new ContextConsumer(this, {
		...
		gatherOutlineConfig() {
			...
			// add all chapters
				for (let i = 0; i < this.studioPageData.content.config.outline.length; i++) {
					...
					case ExtractContentsMode.SUB_HEADING:
							if (hValue === "H4") {
								this.shownOutline.push(this.studioPageData.content.config.outline[i])	
							}
		...
		renderFroalaEditors() {
			if (!this.firstUpdatedPassed)
			this.firstUpdatedPassed = false
			...
			const content = extractContentsUnderOutline(this.settingsData.extractContentsConfig, body, this.shownOutline[i])
			...
			new FroalaEditor(el,
				{
					...
					events: {
						contentChanged: () => {
							const newHTML = el.querySelector("iframe").contentDocument.querySelector("body").innerHTML 
							// now that we have the html, we have to insert this back into the origin html
							// find the element
							const originalHTML = this.studioPageData.content.body.querySelector("body")!

							updateOutlineInBody(newHTML, originalHTML, this.shownOutline[i].id, this.settingsData.extractContentsConfig)
						},
						initialized: () => {
		...
		createNodeElements() {
			...
			const outlineSubtype = convertOutlineToTree(this.studioPageData.content.config.outline)
			...
			const nodes = html`${this.shownOutline.map((outline: Outline, i: number) => { 
				...
				return html`<node-element  style=${styleMap(styles)} edge="h" slot="node">
				<slot name="title">${outline.name}</slot>
				
				<div class="editor-div" id="editor-${outline.id}"></div></node-element>`
			})}`
			...
			const edges = html`${g.edges().map((edgeId: any) => {
				...
				return html`
				<svg style=${styleMap(styles)} class="edge"  width="400" height="200" slot="edge">
					<line x1=${beginningX} y1=${beginningY} x2=${endX} y2=${endY} stroke="black" stroke-width="2"/>
				</svg>
			`
	...
	render() {
		...
		<graph-element .height=${'800px'}>
			<div slot="custom-controls">
				<sl-select value=${this.settingsData.extractContentsConfig.mode} @sl-change=${(e) => {
					const newValue = e.target.value
					
					this.settingsData.extractContentsConfig.mode = newValue
					this.shownOutline = []

					this.dispatchEvent(new CustomEvent(UPDATE_GRAPH_EVENT, { composed: true, bubbles: true}))
					this.dispatchEvent(new CustomEvent<StudioSettings>(SettingsChanges, { detail: this.settingsData, composed: true, bubbles: true}))
